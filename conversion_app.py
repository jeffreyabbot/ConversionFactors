...
import re
import base64
#-m streamlit run conversion_app.py 
...
def calculate_weighted_shooting(history, current_stats, source="FEB", skip_label=None):
	...
	if source == "FEB":
		...
		for row in history:
			try:
				# Skip logic matching by season string (e.g., "2024-25")
				row_season = str(row.get("SEASON", row.get("YEAR", ""))).strip()
				if skip_label and row_season == skip_label:
					continue
...
Spanish Liga U	7	0,78	0,78	0,78	0,80	0,80	0,75	0,85	0,90	1,15	Estimated	-0,25	0,25	77,00	0,97	0,85	0,80
...
def load_scraped_row_into_state(df_row, player_name, height_cm, yob, team_name="Unknown Team"):
	...
	# 3. BLEND SHOOTING WITH HISTORY (Correctly passing the selected season string)
	if rgm_history:
		selected_season = str(df_row.get("SEASON", df_row.get("YEAR", ""))).strip()
		weighted = calculate_weighted_shooting(rgm_history, mapped_stats, source="RealGM", skip_label=selected_season)
...
def run_projection_engine(
	...
	is_defensive_glue = (raw_stats.get("FGA", 0) / minutes) < 0.20
	...
	# FIX: Enforce the rule described in the warning tab
	...
	base_proj = {
		...
		# FIX: Apply situational modifiers (c_mults) to STL and BLK
		"STL": round(((stl10 * (origin_league["STL Modifier"]/dest_league["STL Modifier"]) * pace * role_mult * cont_mult * c_mults["STL"]) / 10.0) * target_min, 1),
		"BLK": round(((blk10 * (origin_league["BLK Modifier"]/dest_league["BLK Modifier"]) * pace * role_mult * cont_mult * c_mults["BLK"]) / 10.0) * target_min, 1),
...
if st.session_state.view_mode == "Home":
	...
	search_query = ""
	selected_index = None
 	# Initialize default analytical variables to satisfy static analysis safety
	target_role_label = "D (Glue / Specialist)"
	origin_role_label = "C/B - Rotational/Core Performer"
	conf_percent = 80
	conf_factor_str = "Baseline stable"
	...
	try:
		...
		raw_pts = st.session_state.raw_stats.get("PTS", 0.0)
		raw_fga = st.session_state.raw_stats.get("FGA", 0.0)
		if raw_pts >= 15.0 or raw_fga >= 12.0:
			origin_role_label = "A - Franchise / Alpha Option"
		elif raw_pts >= 10.0 or raw_fga >= 8.5:
			origin_role_label = "B - Core Starter / Key Contributor"
		elif is_defensive_glue or raw_fga < 6.0:
			origin_role_label = "D - Glue / Specialist / Connector (<6 FGA)"
		else:
			origin_role_label = "C - Rotational Performer"
...
# ----------------- TAB 2: PERFORMANCE PROJECTIONS -----------------
	with tab_projections:
		...
		# Explicitly show that the role mult has been overridden to 1.00
		if is_defensive_glue:
			...
			<div style="background-color: #f8f9fa; padding: 15px; border-radius: 6px; border-left: 5px solid #E63946; margin-bottom: 20px; font-size: 13px; line-height: 1.6;">
				<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px;">
					...
					<div>
						...
						• Leagues: <code style="color:#E63946; font-weight:bold;">{orig_league_name} ➔ {dest_league_name}</code><br/>
						• Roles: <code style="color:#E63946; font-weight:bold;">{origin_role_label.split(' - ')[0]} ➔ {target_role.split(' (')[0]}</code>
					</div>
				</div>
		...
		st.dataframe(
			...
			height=500,
			column_config={
				"Floor": st.column_config.TextColumn(width=150),
				"BASE PROJECTION": st.column_config.TextColumn("PROJECTION", help="The most likely output", width=150),
				"Ceiling": st.column_config.TextColumn(width=150),
				"Active Variance": st.column_config.TextColumn("VAR", width=120)
			}
		)
	...
	with tab_rules_warnings:
		st.markdown("### Qualitative Scouting & Context Analysis")
		
		# Balanced side-by-side dashboard cards
		col_context, col_risk = st.columns(2)
		...
		with col_context:
			with st.container(border=True):
				...
				# Pin to Shortlist button placed cleanly at the bottom of the Context Card
				st.markdown("---")
		...
		if is_defensive_glue:
			st.error("Dynamic Rule - Defensive Glue: Player averaged < 0.20 FGA per minute. Role modifier forced to 1.00")
			warnings += 1
